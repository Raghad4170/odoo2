...
class SaleSubscription(models.Model):
    ...
    stage_id = fields.Many2one(
        'subscription.subscription.stage', string='Stage', index=True, default=lambda s: s._get_default_stage_id(),
    ...
    recurring_invoice_line_ids = fields.One2many('subscription.subscription.line', 'analytic_account_id', string='Subscription Lines', copy=True)
    ...
    close_reason_id = fields.Many2one("subscription.subscription.close.reason", string="Close Reason", copy=False, tracking=True)
    template_id = fields.Many2one(
        'subscription.subscription.template', string='Subscription Template',
    ...
    subscription_log_ids = fields.One2many('subscription.subscription.log', 'subscription_id', string='Subscription Logs', readonly=True)
    ...
    def _get_default_stage_id(self):
        return self.env['subscription.subscription.stage'].search([], order='sequence', limit=1)
    ...
    def _inverse_starred(self):
        starred_subscriptions = not_star_subscriptions = self.env['subscription.subscription'].sudo()
        for subscription in self:
    ...
    def action_open_sales(self):
        ...
        return {
            ...
            "views": [[self.env.ref('subscription_subscription.sale_order_view_tree_subscription').id, "tree"],
    ...
    def _track_subtype(self, init_values):
        ...
        if 'stage_id' in init_values:
            return self.env.ref('subscription_subscription.subtype_stage_change')
    ...
    def create(self, vals):
        vals['code'] = (
            ...
            self.env['ir.sequence'].with_company(vals.get('company_id')).next_by_code('subscription.subscription') or
            'New'
        )
    ...
    def _mail_track(self, tracked_fields, initial):
        ...
        if any(f in updated_fields for f in ['recurring_total', 'template_id']):
            ...
            if not float_is_zero(delta, precision_rounding=cur_round):
                self.env['subscription.subscription.log'].sudo().create({
    ...
    def _get_subscription_delta(self, date):
        ...
        subscription_log = self.env['subscription.subscription.log'].search([
    ...
    def set_close(self):
        ...
        search = self.env['subscription.subscription.stage'].search
        for sub in self:
    ...
    def prepare_renewal_order(self, discard_product_ids=False, new_lines_ids=False):
        ...
        order.message_post(body=(_("This renewal order has been created from the subscription ") + " <a href=# data-oe-model=subscription.subscription data-oe-id=%d>%s</a>" % (self.id, self.display_name)))
    ...
    def start_subscription(self):
        self.ensure_one()
        next_stage_in_progress = self.env['subscription.subscription.stage'].search([('category', '=', 'progress'), ('sequence', '>=', self.stage_id.sequence)], limit=1)
        if not next_stage_in_progress:
            next_stage_in_progress = self.env['subscription.subscription.stage'].search([('category', '=', 'progress')], limit=1)
    ...
    def _recurring_create_invoice(self, automatic=False, batch_size=20):
                        ...
                        try:
                            ...
                            if tx is None or not tx.renewal_allowed:
                                ...
                                if close_subscription:
                                    template = self.env.ref('subscription_subscription.email_payment_close')
                                    ...
                                    template = self.env.ref('subscription_subscription.email_payment_reminder')
        ...
        self._cr.execute('''
            ...
            JOIN subscription_subscription subscr ON subscr.id = aml.subscription_id
            JOIN subscription_subscription_template subscr_tpl ON subscr_tpl.id = subscr.template_id
        ...
        if need_cron_trigger:
            self.env.ref('subscription_subscription.account_analytic_cron_for_invoice')._trigger()
    ...
    def send_success_mail(self, tx, invoice):
        ...
        template = self.env.ref('subscription_subscription.email_payment_success')
...
class SaleSubscriptionLine(models.Model):
    ...
    analytic_account_id = fields.Many2one('subscription.subscription', string='Subscription', ondelete='cascade')
...
class SaleSubscriptionCloseReason(models.Model):
    _name = "subscription.subscription.close.reason"
    _order = "sequence, id"
...
class SaleSubscriptionTemplate(models.Model):
    _name = "subscription.subscription.template"
    ...
    tag_ids = fields.Many2many(
        'account.analytic.tag', 'subscription_subscription_template_tag_rel',
    ...
    invoice_mail_template_id = fields.Many2one(
        ...
        default=lambda self: self.env.ref('subscription_subscription.mail_template_subscription_invoice', raise_if_not_found=False))
    ...
    def _compute_subscription_count(self):
        subscription_data = self.env['subscription.subscription'].read_group(domain=[('template_id', 'in', self.ids), ('stage_id', '!=', False)],
                                                                     fields=['template_id'],
                                                                     groupby=['template_id'])
...
class SaleSubscriptionStage(models.Model):
    _name = 'subscription.subscription.stage'
...
class SaleSubscriptionLog(models.Model):
    _name = 'subscription.subscription.log'
...
class SaleSubscriptionAlert(models.Model):
    ...
    def default_get(self, default_fields):
        ...
        if 'model_id' in default_fields:
            ...
            res['model_id'] = self.env['ir.model']._get_id('subscription.subscription')
        return res
    ...
    subscription_template_ids = fields.Many2many(
        'subscription.subscription.template', string='Subscription Templates',
        check_company=True)
    ...
    stage_from_id = fields.Many2one('subscription.subscription.stage', help="Trigger on changes of stage. Trigger over all stages if not set")
    stage_to_id = fields.Many2one('subscription.subscription.stage')

    tag_id = fields.Many2one('account.analytic.tag', string='Tag')
    stage_id = fields.Many2one('subscription.subscription.stage', string='Stage')
    ...
    def _compute_subscription_count(self):
        for alert in self:
            ...
            alert.subscription_count = self.env['subscription.subscription'].search_count(domain)
    ...
    def run_cron_manually(self):
        ...
        subs = self.env['subscription.subscription'].search(domain)
        ctx = {
    ...
    def _compute_nextcall(self):
        cron = self.env.ref('subscription_subscription.ir_cron_subscription_subscription_update_kpi', raise_if_not_found=False)
        if not cron:
