...
class SaleOrder(models.Model):
    ...
    def action_open_subscriptions(self):
        ...
        action = self.env["ir.actions.actions"]._for_xml_id("subscription_subscription.subscription_subscription_action")
        if len(subscriptions) > 1:
            action['domain'] = [('id', 'in', subscriptions.ids)]
        elif len(subscriptions) == 1:
            form_view = [(self.env.ref('subscription_subscription.subscription_subscription_view_form').id, 'form')]
            if 'views' in action:
    ...
    def _prepare_subscription_data(self, template):
        ...
        recurring_next_date = self.env['subscription.subscription']._get_recurring_next_date(
        ...
        default_stage = self.env['subscription.subscription.stage'].search([('category', '=', 'progress')], limit=1)
        if default_stage:
    ...
    def create_subscriptions(self):
        ...
        for order in self:
            ...
            for template in to_create:
                ...
                subscription = self.env['subscription.subscription'].sudo().create(values)
...
class SaleOrderLine(models.Model):
    ...
    subscription_id = fields.Many2one('subscription.subscription', 'Subscription', copy=False, check_company=True)
        ...
        if self.subscription_id:
            ...
            if self.order_id.subscription_management != 'upsell':  # renewal or creation: one entire period
                ...
                if not date_start or not date_start_display or not date_end:
                    ...
                    # (based on `partial_recurring_invoice_ratio` from the subscription.subscription model)
        ...
        if vals.get('order_id'):
            ...
            if order.origin and order.subscription_management in ('upsell', 'renew') and Product.browse(vals['product_id']).recurring_invoice:
                vals['subscription_id'] = (
                    self.env['subscription.subscription'].search(['&', ('code', '=', order.origin), ('partner_id', '=', order.partner_id.id)], limit=1).id
                    or self.env['subscription.subscription'].search([('code', '=', order.origin)], limit=1).id
                )
