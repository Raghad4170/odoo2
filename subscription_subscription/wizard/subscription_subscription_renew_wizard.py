...
class SaleSubscriptionWizard(models.TransientModel):
    _name = "subscription.subscription.renew.wizard"
    ...
    def _default_products(self):
        ...
        subscription_id = self.env['subscription.subscription'].browse(self._context.get('active_id'))
        sub_default_products = subscription_id.archived_product_ids
        res_ids = []
        Options = self.env['subscription.subscription.renew.wizard.keep_option']
    ...
    kept_archived_product_ids = fields.One2many('subscription.subscription.renew.wizard.keep_option',
                                                'wizard_id',
                                                default=_default_products, string="Archived products",
                                                help="If checked, the product will be reused in the renewal order")
    replacement_line_ids = fields.One2many('subscription.subscription.renew.wizard.replace_option',
...
class SaleSubscriptionRenewWizardKeepOption(models.TransientModel):
    _name = "subscription.subscription.renew.wizard.keep_option"
    ...
    wizard_id = fields.Many2one('subscription.subscription.renew.wizard', required=True, ondelete="cascade")


class SaleSubscriptionRenewWizardReplaceOption(models.TransientModel):
    _name = "subscription.subscription.renew.wizard.replace_option"
    _description = "Subscription Renew Lines Wizard"
