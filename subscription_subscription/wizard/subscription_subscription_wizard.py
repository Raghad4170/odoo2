...
class SaleSubscriptionWizard(models.TransientModel):
    _name = 'subscription.subscription.wizard'
    _description = 'Subscription Upsell wizard'

    def _default_subscription(self):
        return self.env['subscription.subscription'].browse(self._context.get('active_id'))

    subscription_id = fields.Many2one('subscription.subscription', string="Subscription", required=True, default=_default_subscription, ondelete="cascade")
    option_lines = fields.One2many('subscription.subscription.wizard.option', 'wizard_id', string="Options")
    ...
    def create_sale_order(self):
        ...
        order.message_post(body=(_("This upsell order has been created from the subscription ") + " <a href=# data-oe-model=subscription.subscription data-oe-id=%d>%s</a>" % (self.subscription_id.id, self.subscription_id.display_name)))
...
class SaleSubscriptionWizardOption(models.TransientModel):
    _name = "subscription.subscription.wizard.option"
    _description = 'Subscription Upsell Lines Wizard'

    name = fields.Char(string="Description")
    wizard_id = fields.Many2one('subscription.subscription.wizard', required=True, ondelete="cascade")
