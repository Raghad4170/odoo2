...
class subscription_subscription_report(models.Model):
    _name = "subscription.subscription.report"
    ...
    template_id = fields.Many2one('subscription.subscription.template', 'Subscription Template', readonly=True)
    ...
    close_reason_id = fields.Many2one('subscription.subscription.close.reason', 'Close Reason', readonly=True)
    ...
    stage_id = fields.Many2one('subscription.subscription.stage', string='Stage', readonly=True)
    ...
    def _from(self):
        from_str = """
                subscription_subscription_line l
                      join subscription_subscription sub on (l.analytic_account_id=sub.id)
                      join subscription_subscription_stage stage on sub.stage_id = stage.id
