odoo.define('penal.type.selection', function (require) {
"use strict";
                    ...
                    self.hierarchy_groups = [
                        {
                            'name': _t('جزائية'),
                            'children': [
                                {'name': _t('الطلبات القضائية'), 'ids': _.map(_.filter(arg, v => v['type'] == '1'), v => v['id'])},
                                {'name': _t('تعزير'), 'ids': _.map(_.filter(arg, v => v['type'] == '2'), v => v['id'])},
                                {'name': _t('حدود'), 'ids': _.map(_.filter(arg, v => v['type'] == '3'), v => v['id'])},
                                {'name': _t('قصاص'), 'ids': _.map(_.filter(arg, v => v['type'] == '4'), v => v['id'])},
                                {'name': _t('مطالبة مالية'), 'ids': _.map(_.filter(arg, v => v['type'] == '5'), v => v['id'])},
                            ],
                        },
                    ]
                });
    ...
    registry.add("penal_type_selection", HierarchySelection);
});
