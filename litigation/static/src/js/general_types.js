odoo.define('general.type.selection', function (require) {
"use strict";
                    ...
                    self.hierarchy_groups = [
                        {
                            'name': _t('عامة'),
                            'children': [
                                {'name': _t('إنهاءات'), 'ids': _.map(_.filter(arg, v => v['type'] == '1'), v => v['id'])},
                                {'name': _t('الدعاوى المستعجلة'), 'ids': _.map(_.filter(arg, v => v['type'] == '2'), v => v['id'])},
                                {'name': _t('عقارية'), 'ids': _.map(_.filter(arg, v => v['type'] == '3'), v => v['id'])},
                                {'name': _t('مالية'), 'ids': _.map(_.filter(arg, v => v['type'] == '4'), v => v['id'])},
                                {'name': _t('مروري'), 'ids': _.map(_.filter(arg, v => v['type'] == '5'), v => v['id'])},
                                {'name': _t('أخرى'), 'ids': _.map(_.filter(arg, v => v['type'] == '6'), v => v['id'])},
                            ],
                        },
                    ]
                });
    ...
    registry.add("general_type_selection", HierarchySelection);
});
