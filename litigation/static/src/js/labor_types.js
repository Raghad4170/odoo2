odoo.define('labor.type.selection', function (require) {
"use strict";
                    ...
                    self.hierarchy_groups = [
                        {
                            'name': _t('عمالية'), 
                            'children': [
                                {'name': _t('اعتراض على قرارات'), 'ids': _.map(_.filter(arg, v => v['type'] == '1'), v => v['id'])},
                                {'name': _t('الطلبات العارضة والعاجلة'), 'ids': _.map(_.filter(arg, v => v['type'] == '2'), v => v['id'])},
                                {'name': _t('حقوق مالية'), 'ids': _.map(_.filter(arg, v => v['type'] == '3'), v => v['id'])},
                                {'name': _t('حقوق وظيفية'), 'ids': _.map(_.filter(arg, v => v['type'] == '4'), v => v['id'])},
                                {'name': _t('مستندات ووثائق'), 'ids': _.map(_.filter(arg, v => v['type'] == '5'), v => v['id'])},
                                {'name': _t('تعويض'), 'ids': _.map(_.filter(arg, v => v['type'] == '6'), v => v['id'])},
                            ],
                        },
                    ]
                });
    ...
    registry.add("labor_type_selection", HierarchySelection);
});
