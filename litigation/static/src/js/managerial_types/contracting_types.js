odoo.define('contracting.type.selection', function (require) {
"use strict";
                    ...
                    self.hierarchy_groups = [
                        {
                            'name': _t('عقد'),
                            'children': [
                                {'name': _t('إجراءات ترسية'), 'ids': _.map(_.filter(arg, v => v['type'] == '1'), v => v['id'])},
                                {'name': _t('مسؤولية عقدية'), 'ids': _.map(_.filter(arg, v => v['type'] == '2'), v => v['id'])},
                           ],
                        },
                    ]
                });
    ...
    registry.add("contracting_type_selection", HierarchySelection);
});
