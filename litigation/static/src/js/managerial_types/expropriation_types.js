odoo.define('expropriation.type.selection', function (require) {
"use strict";
                    ...
                    self.hierarchy_groups = [
                        {
                            'name': _t('نزع ملكية'),
                            'children': [
                                {'name': _t('نزع عقار'), 'ids': _.map(_.filter(arg, v => v['type'] == '1'), v => v['id'])},
                                {'name': _t('تعويض'), 'ids': _.map(_.filter(arg, v => v['type'] == '2'), v => v['id'])},
                                {'name': _t('إعادة العقار المنزوع بعد استغناء الجهة'), 'ids': _.map(_.filter(arg, v => v['type'] == '3'), v => v['id'])},
                            ],
                        },
                    ]
                });
    ...
    registry.add("expropriation_type_selection", HierarchySelection);
});
