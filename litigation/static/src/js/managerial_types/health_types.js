odoo.define('health.type.selection', function (require) {
"use strict";
                    ...
                    self.hierarchy_groups = [
                        {
                            'name': _t('صحية'),
                            'children': [
                                {'name': _t('مؤسسات صحية'), 'ids': _.map(_.filter(arg, v => v['type'] == '1'), v => v['id'])},
                                {'name': _t('مهن صحية'), 'ids': _.map(_.filter(arg, v => v['type'] == '2'), v => v['id'])},
                                {'name': _t('المنشآت والمستحضرات الصيدلانية'), 'ids': _.map(_.filter(arg, v => v['type'] == '3'), v => v['id'])},
                                {'name': _t('طبي'), 'ids': _.map(_.filter(arg, v => v['type'] == '4'), v => v['id'])},
                            ],
                        },
                    ]
                });
    ...
    registry.add("health_type_selection", HierarchySelection);
});
