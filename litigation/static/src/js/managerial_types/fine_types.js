odoo.define('fine.type.selection', function (require) {
"use strict";
                    ...
                    self.hierarchy_groups = [
                        {
                            'name': _t('بلدية'),
                            'children': [
                                {'name': _t('مخالفات البناء'), 'ids': _.map(_.filter(arg, v => v['type'] == '6.1'), v => v['id'])},
                                {'name': _t('مخالفات البيع'), 'ids': _.map(_.filter(arg, v => v['type'] == '6.2'), v => v['id'])},
                                {'name': _t('مخالفات الذبح والمواشي'), 'ids': _.map(_.filter(arg, v => v['type'] == '6.3'), v => v['id'])},
                                {'name': _t('مخالفات الصحة العامة'), 'ids': _.map(_.filter(arg, v => v['type'] == '6.4'), v => v['id'])},
                                {'name': _t('مخالفات الطرق'), 'ids': _.map(_.filter(arg, v => v['type'] == '6.5'), v => v['id'])},
                            ],
                        },
                    ]
                });
    ...
    registry.add("fine_type_selection", HierarchySelection);
});
