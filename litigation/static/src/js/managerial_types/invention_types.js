odoo.define('invention.type.selection', function (require) {
"use strict";
                    ...
                    self.hierarchy_groups = [
                        {
                            'name': _t('ملكية فكرية'),
                            'children': [
                                {'name': _t('اختراع'), 'ids': _.map(_.filter(arg, v => v['type'] == '1'), v => v['id'])},
                                {'name': _t('تصميمات تخطيط للدارات المتكاملة'), 'ids': _.map(_.filter(arg, v => v['type'] == '2'), v => v['id'])},
                                {'name': _t('أصناف نباتية'), 'ids': _.map(_.filter(arg, v => v['type'] == '3'), v => v['id'])},
                                {'name': _t('نماذج صناعية'), 'ids': _.map(_.filter(arg, v => v['type'] == '4'), v => v['id'])},
                                {'name': _t('علامة تجارية'), 'ids': _.map(_.filter(arg, v => v['type'] == '5'), v => v['id'])},
                                {'name': _t('اسم تجاري'), 'ids': _.map(_.filter(arg, v => v['type'] == '6'), v => v['id'])},
                                {'name': _t('حقوق المؤلف'), 'ids': _.map(_.filter(arg, v => v['type'] == '7'), v => v['id'])},
                            ],
                        },
                    ]
                });
    ...
    registry.add("invention_type_selection", HierarchySelection);
});
