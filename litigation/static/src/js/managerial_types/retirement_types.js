odoo.define('retirement.type.selection', function (require) {
"use strict";
                    ...
                    self.hierarchy_groups = [
                        {
                            'name': _t('تقاعد'),
                            'children': [
                                {'name': _t('مدني'), 'ids': _.map(_.filter(arg, v => v['type'] == '1'), v => v['id'])},
                                {'name': _t('عسكري'), 'ids': _.map(_.filter(arg, v => v['type'] == '2'), v => v['id'])},
                           ],
                        },
                    ]
                });
    ...
    registry.add("retirement_type_selection", HierarchySelection);
});
