odoo.define('judicial.type.selection', function (require) {
"use strict";
            ...
            if (!self.hierarchy_groups) {
                prom = this._rpc({
                    ...
                    self.hierarchy_groups = [
                        {
                            'name': _t('طلبات قضائية')
                         },
                        {
                            'name': _t('تصحيح الحكم'), 'ids': _.map(_.filter(arg, v => v['type'] == '1'), v => v['id'])
                         },
                        {
                            'name': _t('تفسير الحكم'), 'ids': _.map(_.filter(arg, v => v['type'] == '2'), v => v['id'])
                         },
                        {
                            'name': _t('طلب عاجل'), 'ids': _.map(_.filter(arg, v => v['type'] == '3'), v => v['id'])
                         },
                        ...
                        
                        {
                            'name': _t('إعادة نظر'),
                            'children': [
                                {'name': _t('في حكم ابتدائي'), 'ids': _.map(_.filter(arg, v => v['type'] == '4'), v => v['id'])},
                                {'name': _t('في حكم استئناف'), 'ids': _.map(_.filter(arg, v => v['type'] == '5'), v => v['id'])},
                            ],
                        },
                    ]
                });
    ...
    registry.add("judicial_type_selection", HierarchySelection);
});
