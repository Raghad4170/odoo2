odoo.define('discipline.type.selection', function (require) {
"use strict";
                    ...
                    self.hierarchy_groups = [
                        {
                            'name': _t('تأديب'),
                            'children': [
                                {'name': _t('دعوى تأديبية'), 'ids': _.map(_.filter(arg, v => v['type'] == '1'), v => v['id'])},
                                {'name': _t('قرارات تأديبية'), 'ids': _.map(_.filter(arg, v => v['type'] == '2'), v => v['id'])},
                           ],
                        },
                    ]
                });
    ...
    registry.add("discipline_type_selection", HierarchySelection);
});
